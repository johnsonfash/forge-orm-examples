...
export function App() {
  const [results, setResults] = React.useState<Array<{ product: string; revenue: number }>>([])
  ...
  React.useEffect(() => {
    (async () => {
      ...
      setResults(top.map((r) => ({ product: r.product, revenue: r._sum.qty ?? 0 })))
      setLoading(false)
    })()
  }, [])
  ...
  return (
    <div style={{ maxWidth: 480, margin: "40px auto", fontFamily: "system-ui" }}>
      ...
      <ul>
        {results.map((r) => (
          <li key={r.product}>{r.product} — {r.revenue} units</li>
        ))}
      </ul>
